 ...
 */
package com.b2international.index.os.admin;
...
import org.opensearch.action.admin.cluster.health.ClusterHealthRequest.Level;
...
import com.b2international.index.os.query.OsQueryBuilder;
 ...
 * @since 5.10
 */
public final class OsIndexAdmin implements IndexAdmin {
	...
	public OsIndexAdmin(OsClient client, ObjectMapper mapper, String name, Mappings mappings, Map<String, Object> settings) {
		this.client = client;
		this.mapper = mapper;
	...
	public boolean exists() {
		try {
			...
			throw new IndexException("Couldn't check the existence of all OS indices.", e);
		}
	}
	...
	private boolean exists(DocumentMapping mapping) {
		...
		try {
			...
			throw new IndexException("Couldn't check the existence of OS index '" + index + "'.", e);
		}
	}
			...
			if (exists(mapping)) {
				...
				try {
					...
					if (!incompatibleChanges.isEmpty()) {
						log.warn("Cannot migrate index '{}' to new mapping with breaking changes on properties '{}'. Run repository reindex to migrate to new mapping schema or drop that index manually using the opensearch API.", index, incompatibleChanges);
	...
	private Map<String, Object> toProperties(DocumentMapping mapping) {
		...
		for (Field field : mapping.getFields()) {
			...
			if (Map.class.isAssignableFrom(fieldType)) {
				...
				final Map<String, Object> prop = newHashMap();
				// XXX type: object is the default for nested objects, OS won't store it in the mapping and will default to object even if explicitly set, which would cause unnecessary mapping update during boot
				if (annotation.nested()) {
					prop.put("type", "nested");
				}
				// XXX enabled: true is the default, OS won't store it in the mapping and will default to true even if explicitly set, which would cause unnecessary mapping update during boot
				if (!annotation.index()) {
				...
				if (!mapping.isText(property) && !mapping.isKeyword(property)) {
					...
					if (keywordMapping != null) {
						...
						// XXX index: true is the default, OS won't store it in the mapping and will default to true even if explicitly set, which would cause unnecessary mapping update during boot
						// XXX doc_values: true is the default, OS won't store it in the mapping and will default to true even if explicitly set, which would cause unnecessary mapping update during boot
	...
	public void delete() {
		if (exists()) {
			...
			try {
				...
				checkState(deleteIndexResponse.isAcknowledged(), "Failed to delete all OS indices for '%s'.", name);
			} catch (Exception e) {
				throw new IndexException(String.format("Failed to delete all OS indices for '%s'.", name), e);
			}
		}
	}
...
//		waitForYellowHealth();
	...
	public int maxTermsCount() {
		return Integer.parseInt((String) settings().get(IndexClientFactory.MAX_TERMS_COUNT_KEY));
	}
	...
	public void refresh(Set<DocumentMapping> typesToRefresh) {
		if (!CompareUtils.isEmpty(typesToRefresh)) {
			...
			try {
				...
				throw new IndexException(String.format("Failed to refresh OS indexes '%s'.", Arrays.toString(indicesToRefresh)), e);
			}
		}
	}
	...
	public boolean bulkUpdate(final BulkUpdate<?> update) {
		...
		org.opensearch.script.Script script = new org.opensearch.script.Script(ScriptType.INLINE, "painless", rawScript, Map.copyOf(update.getParams()));
	...
	private boolean bulkIndexByScroll(final OsClient client,
		...
		final QueryBuilder query = new OsQueryBuilder(mapping, settings, log).build(filter);
