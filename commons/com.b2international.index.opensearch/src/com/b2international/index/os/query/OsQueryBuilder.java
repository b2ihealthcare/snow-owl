 ...
 */
package com.b2international.index.os.query;
...
import com.b2international.index.os.OsScriptFactory;
 ...
 * @since 4.7
 */
public final class OsQueryBuilder {
	...
	public OsQueryBuilder(DocumentMapping mapping, Map<String, Object> settings, Logger log) {
		this(mapping, settings, log, "");
	}
	
	private OsQueryBuilder(DocumentMapping mapping, Map<String, Object> settings, Logger log, String path) {
	...
	private void visit(ScriptQueryExpression expression) {
		deque.push(QueryBuilders.scriptQuery(OsScriptFactory.INSTANCE.toOsScript(expression, mapping)));
	}
	...
	private void visit(ScriptScoreExpression expression) {
		...
		deque.push(QueryBuilders
				.functionScoreQuery(innerQuery, ScoreFunctionBuilders.scriptFunction(OsScriptFactory.INSTANCE.toOsScript(expression, mapping)))
	...
	private void visit(BoolExpression bool) {
		...
		for (Expression must : bool.mustClauses()) {
			...
			final OsQueryBuilder innerQueryBuilder = new OsQueryBuilder(mapping, settings, log);
	...
	private void visit(NestedPredicate predicate) {
		...
		final OsQueryBuilder nestedQueryBuilder = new OsQueryBuilder(nestedMapping, settings, log, nestedPath);
