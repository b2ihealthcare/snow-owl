 ...
 * @since 7.2
 */
public abstract class OsClientBase implements OsClient {
	...
	public OsClientBase(String clusterUrl) {
	...
	public final ClusterStatus status(String...indices) {
		...
		if (available) {
			...
			for (String index : indices == null || indices.length == 0 ? clusterHealth.get().getIndices().keySet() : Arrays.asList(indices)) {
				...
				final IndexStatus.HealthStatus convertedHealth;
				switch (indexHealth) {
					case RED: convertedHealth = IndexStatus.HealthStatus.RED; break;
					case YELLOW: convertedHealth = IndexStatus.HealthStatus.YELLOW; break;
					case GREEN: convertedHealth = IndexStatus.HealthStatus.GREEN; break;
					default: convertedHealth = IndexStatus.HealthStatus.RED; break;
				}
				
				indexStatuses.add(new IndexStatus(index, convertedHealth, diagnosis));
			}
			return new ClusterStatus(available, clusterDiagnosis, indexStatuses);
		} else {
			return new ClusterStatus(available, clusterDiagnosis, Collections.emptyList());
		}
	}
	...
	private String checkClusterAvailable(String previousAvailability) {
		try {
			...
			if (e instanceof OpenSearchStatusException && ((OpenSearchStatusException) e).status() == RestStatus.UNAUTHORIZED) {
