...
public final class IndicesTcpClient implements IndicesClient {
	...
	public boolean exists(String...indices) throws IOException {
		return OsTcpClient.execute(client.prepareExists(indices).execute()).isExists();
	}
	...
	public CreateIndexResponse create(CreateIndexRequest req) throws IOException {
		...
		var tcpReq = new org.opensearch.action.admin.indices.create.CreateIndexRequest(req.index())
			.mapping("_doc", req.mappings().utf8ToString(), req.mappingsXContentType())
			.settings(req.settings());
		var tcpResp = OsTcpClient.execute(client.create(tcpReq));
	...
	public AcknowledgedResponse delete(DeleteIndexRequest req) throws IOException {
		return OsTcpClient.execute(client.delete(req));
	}

	@Override
	public RefreshResponse refresh(RefreshRequest req) throws IOException {
		return OsTcpClient.execute(client.refresh(req));
	}
	...
	public GetMappingsResponse getMapping(GetMappingsRequest req) throws IOException {
		...
		var tcpReq = new org.opensearch.action.admin.indices.mapping.get.GetMappingsRequest()
			.indices(req.indices());
		var tcpResp = OsTcpClient.execute(client.getMappings(tcpReq));
	...
	public AcknowledgedResponse updateMapping(PutMappingRequest req) throws IOException {
		 ...
		 */
		var tcpReq = new org.opensearch.action.admin.indices.mapping.put.PutMappingRequest(req.indices())
			.source(req.source(), req.xContentType())
			.type("_doc");
		var tcpResp = OsTcpClient.execute(client.putMapping(tcpReq));
	...
	public GetSettingsResponse settings(GetSettingsRequest req) throws IOException {
		return OsTcpClient.execute(client.getSettings(req));
	}
	
	@Override
	public AcknowledgedResponse updateSettings(UpdateSettingsRequest req) throws IOException {
		return OsTcpClient.execute(client.updateSettings(req));
	}
}
