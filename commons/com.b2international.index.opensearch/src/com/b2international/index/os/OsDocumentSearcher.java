 ...
 * @since 5.10
 */
public class OsDocumentSearcher implements Searcher {
	...
	public OsDocumentSearcher(OsIndexAdmin admin, ObjectMapper mapper) {
		this.admin = admin;
		this.mapper = mapper;
		this.resultWindow = Integer.parseInt((String) admin.settings().get(IndexClientFactory.RESULT_WINDOW_KEY));
		this.maxTermsCount = admin.maxTermsCount();
	}
	...
	public <T> Hits<T> search(Query<T> query) throws IOException {
		final OsClient client = admin.client();
		...
		final OsQueryBuilder esQueryBuilder = new OsQueryBuilder(mapping, admin.settings(), admin.log());
		...
		// OS internals require loading the _id field when we require the _source
...
//			req.scroll(scrollTime);
	...
	public <T> Hits<T> scroll(Scroll<T> scroll) throws IOException {
		...
		try {
			...
			if (rootCause instanceof OpenSearchException && rootCause.getMessage().contains("No search context found for id [")) {
	...
	private <T> Hits<T> toHits(
		...
		final OsHitConverter<T> hitConverter = OsHitConverter.getConverter(mapper, select, from, fetchSource, fields);
	...
	private void addSort(DocumentMapping mapping, SearchSourceBuilder reqSource, SortBy sortBy, boolean liveScroll) {
		for (final SortBy item : getSortFields(sortBy)) {
			if (item instanceof SortByField) {
				...
				reqSource.sort(SortBuilders.scriptSort(OsScriptFactory.INSTANCE.toOsScript(sortByScript, mapping), ScriptSortType.STRING)
						.order(sortOrder));
	...
	public <T> Aggregation<T> aggregate(AggregationBuilder<T> aggregation) throws IOException {
		...
		reqSource.aggregation(toOsAggregation(mapping, aggregation, fetchSource));
		...
		for (org.opensearch.search.aggregations.bucket.terms.Terms.Bucket bucket : aggregationResult.getBuckets()) {
	...
	private org.opensearch.search.aggregations.AggregationBuilder toOsAggregation(DocumentMapping mapping, AggregationBuilder<?> aggregation, boolean fetchSource) {
		...
		if (isFieldAgg) {
			...
			termsAgg.script(OsScriptFactory.INSTANCE.toOsScript(aggregation, mapping));
		} else {
