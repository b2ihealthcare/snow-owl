...
/**
 * An {@link IProgressMonitor} implementation that displays progress using a Log4j {@link Logger}.  
 *
 */
public class LogProgressMonitorWrapper extends ProgressMonitorWrapper {
	
	private static IProgressMonitor nullToNullMonitor(final IProgressMonitor wrappedMonitor) {
	...
	public LogProgressMonitorWrapper(final IProgressMonitor wrappedMonitor, final Logger logger) {
	 ...
	 */
	public LogProgressMonitorWrapper(final IProgressMonitor wrappedMonitor, final Logger logger, final int maxEchoes) {
	...
	protected void logWork(final int percent) {
		if (maxEchoes <= 0 || numEchoes <= 0 || numEchoes >= totalWork / maxEchoes) {
			logger.info("{}: {}", name, String.format("%d%%",percent));
			numEchoes = 0;
		}
	}
	...
	public void subTask(final String subTask) {
		try {
			...
			logger.info("{}: starting subtask '{}'", name, subTask);
		}
	}
