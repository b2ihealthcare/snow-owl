 ...
 * 
 * B2i Healthcare - remove Lucene-specific API and methods, formatting, add isEmpty method
 */
...
/**
 * A bit set that only stores longs that have at least one bit which is set. The
 * way it works is that the space of bits is divided into blocks of 4096 bits,
 * which is 64 longs. Then for each block, we have:
 *
 * <ul>
 * <li>a long[] which stores the non-zero longs for that block
 * <li>a long so that bit <code>i</code> being set means that the
 * <code>i-th</code> long of the block is non-null, and its offset in the array
 * of longs is the number of one bits on the right of the <code>i-th</code> bit.
 * </ul>
 */
public class SparseFixedBitSet {
	...
	private void or(final int i4096, final long index, long[] bits, int nonZeroLongCount) {
		...
		if (currentIndex == 0) {
			...
			indices[i4096] = index;
			this.bits[i4096] = Arrays.copyOfRange(bits, 0, nonZeroLongCount);
		...
		// we iterate backwards in order to not override data we might need on the next
		// iteration if the
		// array is reused
		for (int i = Long.numberOfLeadingZeros(newIndex), newO = Long.bitCount(newIndex) - 1; i < 64; i += 1
				+ Long.numberOfLeadingZeros(newIndex << (i + 1)), newO -= 1) {
	...
	public void or(SparseFixedBitSet other) {
	...
	public boolean isEmpty() {
		return approximateCardinality() <= 0;
	}
}
