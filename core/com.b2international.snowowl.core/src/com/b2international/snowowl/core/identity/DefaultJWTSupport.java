 ...
 * @since 8.8.0
 */
class DefaultJWTSupport implements JWTSupport {
	...
	private static final Map<String, BiFunction<JWTConfiguration, RSAKeyProvider, Algorithm>> SUPPORTED_JWS_ALGORITHMS = Map.of(
	...
	public DefaultJWTSupport(String identityProviderType, JWTConfiguration config) {
	...
	public void init() throws Exception {
		...
		if (!Strings.isNullOrEmpty(config.getJws())) {
			algorithm = SUPPORTED_JWS_ALGORITHMS.getOrDefault(config.getJws(), this::throwUnsupportedJws).apply(config, rsaKeyProvider);
		} else {
			IdentityProvider.LOG.warn("'identity.providers.{}.jwt.jws' configuration is missing, disabling JWT authorization token signing and verification.", identityProviderType);
			algorithm = null;
		}
	...
	private com.auth0.jwt.JWTVerifier createJWTVerifier(Algorithm algorithm, final JWTConfiguration conf) {
	...
	private RSAKeyProvider createRSAKeyProvider(JWTConfiguration conf) throws Exception {
		...
		return createRSAKeyProvider(conf, privateKeyId, privateKey);
	}

	protected RSAKeyProvider createRSAKeyProvider(JWTConfiguration conf, final String privateKeyId, final RSAPrivateKey privateKey) throws Exception {
	...
	private Algorithm throwUnsupportedJws(JWTConfiguration config, RSAKeyProvider keyProvider) {
