...
/**
 * Simple {@link PropertyFilter} implementation that only uses property name to
 ...
 * {@link PropertyFilter} implementations is strongly encouraged, because it can
 * provide default implementation for any methods that may be added in
 * {@link PropertyFilter} (as unfortunate as additions may be).
 */
@SuppressWarnings("deprecation")
public class FhirPropertyFilter implements PropertyFilter
// sub-classes must also implement java.io.Serializable
{
	...
	protected FhirPropertyFilter() {
	}

	/**
	 * Convenience factory method that will return a "no-op" filter that will simply
	 * just serialize all properties that are given, and filter out nothing.
	 *
	 * @since 2.6
	 */
	public static FhirPropertyFilter serializeAll() {
		return SerializeExceptFilter.INCLUDE_ALL;
	}

	/**
	 * Factory method that was accidentally added in 2.5 with arguments; basically
	 * works just as an alias of {@link #filterOutAllExcept(Set)} which is not very
	 * useful. Instead, see {@link #serializeAll()} for intended signature.
	 *
	 * @deprecated Since 2.6; to be removed from 2.7
	 */
	@Deprecated
	public static FhirPropertyFilter serializeAll(Set<String> properties) {
	 ...
	 */
	public static FhirPropertyFilter filterOutAllExcept(Set<String> properties) {
		return new FilterExceptFilter(properties);
	}

	public static FhirPropertyFilter filterOutAllExcept(String... propertyArray) {
		HashSet<String> properties = new HashSet<String>(propertyArray.length);
		Collections.addAll(properties, propertyArray);
		return new FilterExceptFilter(properties);
	}

	public static FhirPropertyFilter serializeAllExcept(Set<String> properties) {
		return new SerializeExceptFilter(properties);
	}

	public static FhirPropertyFilter serializeAllExcept(String... propertyArray) {
		HashSet<String> properties = new HashSet<String>(propertyArray.length);
		Collections.addAll(properties, propertyArray);
		return new SerializeExceptFilter(properties);
	}
...
//	}
	...
	*/
	...
	/**
	 * Filter implementation which defaults to filtering out unknown properties and
	 * only serializes ones explicitly listed.
	 */
	public static class FilterExceptFilter extends FhirPropertyFilter implements java.io.Serializable {
	...
	/**
	 * Filter implementation which defaults to serializing all properties, except
	 * for ones explicitly listed to be filtered out.
	 */
	public static class SerializeExceptFilter extends FhirPropertyFilter implements java.io.Serializable {
		private static final long serialVersionUID = 1L;
