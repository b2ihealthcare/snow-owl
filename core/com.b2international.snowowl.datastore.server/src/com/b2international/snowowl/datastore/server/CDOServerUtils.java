 ...
 * <p>
 * <b>NOTE:&nbsp;</b>This class is heavily depends on server side services registered to the application context. Be smart when moving this class to
 * other plug-in.
 */
...
public abstract class CDOServerUtils {
	 ...
	 */
	public static CDOCommitInfo commit(final CDOEditingContext context, final String userId, @Nullable final String comment,
	 ...
	 * @param monitor
	 *            monitor for the progress. Can be {@code null}.
	 ...
	 * @param comment
	 *            comment for the commit.
	 * @param notifyWriteAccessHandlers
	 *            {@code true} if all registered {@link WriteAccessHandler} registered to the {@link IRepository} instance should be notified after a
	 *            successful backend commit, otherwise {@code false}.
	 * @param monitor
	 *            monitor for the progress. Can be {@code null}.
	 * @return the commit information containing the changes generated by persisting the change set wrapped by the current transaction.
	 * @throws CommitException
	 ...
	 * Commits the specified transaction with someone else user ID to the backend.
	 * 
	 * @param transactionAggregator
	 *            an aggregator encapsulating an arbitrary number of CDO transactions.
	 * @param userId
	 *            the unique ID of the user.
	 * @param comment
	 *            comment for the commit.
	 ...
	 *            {@code true} if all registered {@link WriteAccessHandler} registered to the {@link IRepository} instance should be notified after a
	 *            successful backend commit, otherwise {@code false}.
	 * @param sendCommitNotification
	 *            {@code true} is {@link InternalRepository#sendCommitNotification(InternalSession, CDOCommitInfo)} should be invoked after successful
	 *            commit, otherwise {@code false}.
	 * @param monitor
	 *            monitor for the progress. Can be {@code null}.
	 * @return the commit information containing the changes generated by persisting the change set wrapped by the current transaction.
	 * @throws CommitException
	 *             if the commit failed.
	 */
	public static Iterable<CDOCommitInfo> commit(final ICDOTransactionAggregator transactionAggregator, final String userId,
			@Nullable final String comment, final boolean notifyWriteAccessHandlers, @Nullable final IProgressMonitor monitor)
			throws CommitException {
	 ...
	 */
	public static Iterable<CDOCommitInfo> commit(final ICDOTransactionAggregator transactionAggregator, final String userId,
			@Nullable final String comment, final boolean notifyWriteAccessHandlers, final boolean sendCommitNotification,
			@Nullable IProgressMonitor monitor) throws CommitException {

		return new CDOServerCommitBuilder(userId, comment, transactionAggregator).notifyWriteAccessHandlers(notifyWriteAccessHandlers)
				.sendCommitNotification(sendCommitNotification).commit(monitor);
	}

	/**
	 * Performs a {@link InternalRepository#sendCommitNotification(InternalSession, CDOCommitInfo)} on the {@link InternalRepository repository}
	 * associated with the {@link CDOCommitInfo commit info} argument. <br>
	 * This method does nothing if the {@link CDOCommitInfo commit info} argument is either {@code null} or representing a failed commit.
	 * 
	 * @param commitInfo
	 *            the commit info to send.
	 */
	 ...
	 * 
	 * @param branchPoint
	 *            the branch point to get the revisions.
	 * @param ids
	 *            the CDO IDs.
	...
	/**
	 * Returns with the revision of an object given by its unique storage key from the {@link CDOBranchVersion branch version}. <br>
	 * May return with {@code null} if the object cannot be found.
	 * 
	 * @param branchVersion
	 *            the version of a CDO branch.
	 * @param id
	 *            the unique CDO ID of an object.
	 * @return the revision of the object or {@code null}.
	 */
	@Nullable
	public static CDORevision getRevision(final CDOBranchVersion branchVersion, final CDOID id) {
		...
		try {
			...
			return CDOUtils.getRevisionManager(id).getRevisionByVersion(id, branchVersion, CDORevision.UNCHUNKED, true);
	...
	/**
	 * Returns with a list of revisions representing the whole life of an object. Each {@link CDORevision revision} represents the actual state of an
	 * object. The list is ordered by time-wise, in other words, the first element representing the latest state of the object, the last element is
	 * representing the birth of the object given by its unique CDO ID. May return with an empty list, if the object never existed on the specified
	 * branch before (inclusive to the branch point timestamp) the branch point.
	 * <p>
	 * This method equivalent to {@link #getObjectRevisions(CDOBranchPoint, CDOID, Integer.MAX_VALUE)}.
	 * 
	 * @param branchPoint
	 *            the branch point representing the start point.
	 * @param cdoId
	 *            the unique ID of the object.
	 * @return a list of revisions, representing lifecycle of an object. Could be empty list if the object cannot be found.
	...
	/**
	 * This method behaves equivalently to {@link #getObjectRevisions(CDOBranchPoint, CDOID)} but stops collecting the revisions (representing the
	 * object lifecycle states) after it reaches the limit.
	 * <p>
	 * This could be a more efficient than {@link #getObjectRevisions(CDOBranchPoint, CDOID)} when client has the CDO ID of a detached object but has
	 * no idea when was it deleted. And only the latest existing state is required.
	 * <p>
	 * Or the client has an existing object but would like to compare the current state of that object with the previous state.
	 * 
	 * @param branchPoint
	 *            the branch point.
	 * @param cdoId
	 *            the unique CDO ID of an object.
	 * @param limit
	 *            the maximum number of CDO revisions to return.
	...
	public static List<CDORevision> getObjectRevisions(final CDOBranchPoint branchPoint, final CDOID cdoId, final int limit) {
		...
		if (emptyOrNullRevision(revisions)) {
			...
			((CommitInfoLoader) getAccessor(cdoId)).loadCommitInfos(branchPoint.getBranch(), // branch
	 ...
	 */
	@Nullable
	public static Throwable checkDuplicateResources(final CDORevision revision, final CDOBranchPoint targetBranchPoint) {
	...
	/**
	 * Returns with the last commit time made on the given branch. <br>
	 * This method will return with {@link Long#MIN_VALUE} if no modification has been made on the given branch, hence no commit time is associated
	 * with it.
	 * 
	 * @param branch
	 * @return
	...
	/**
	 * Returns with the store accessor for a given repository.<br>
	 * Never {@code null}.
	 */
	...
	/**
	 * Returns with the store accessor for an object given as its CDO ID argument.<br>
	 * Never {@code null}.
	 */
	 ...
	 */
	public static IRepository createRepository(final String name, final IStore store, final Map<String, String> properties,
			final IErrorLoggingStrategy strategy) {
		...
		final Repository repository = new Repository.Default() {
			@Override
			public InternalCommitContext createCommitContext(final InternalTransaction transaction) {
	...
	private static final LoadingCache<String, IDBStoreAccessor> ACCESSOR_CACHE = CacheBuilder.newBuilder()
			.build(new CacheLoader<String, IDBStoreAccessor>() {
				@Override
				public IDBStoreAccessor load(final String uuid) throws Exception {
