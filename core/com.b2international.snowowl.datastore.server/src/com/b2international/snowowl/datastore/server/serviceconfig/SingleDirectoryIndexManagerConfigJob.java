...
import com.b2international.snowowl.datastore.server.index.SingleDirectoryIndexServiceManager;
import com.b2international.snowowl.datastore.serviceconfig.ServiceConfigJob;

/**
 *
 */
public class SingleDirectoryIndexManagerConfigJob extends ServiceConfigJob {

	private static final String JOB_NAME = "Supplementary index service configuration...";

	public SingleDirectoryIndexManagerConfigJob() {
	...
	protected boolean initService() throws SnowowlServiceException {
		...
		final ISingleDirectoryIndexServiceManager service = new SingleDirectoryIndexServiceManager();
		ApplicationContext.getInstance().registerService(ISingleDirectoryIndexServiceManager.class, service);
		return true;
	}
}