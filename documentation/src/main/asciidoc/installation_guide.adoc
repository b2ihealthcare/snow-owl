...
- B2i Healthcare only officially supports Snow Owl running on 64-bit derivatives of x86 hardware. 
...
.^|LDAP .^|ApacheDS .^|2.0.M12|
- Authentication and authorization of browsers, terminology editors, reviewers and administrators is performed through an LDAP server. 

- The package to install can be located by following the download link after `ApacheDS 2.0.0-M12` on the http://directory.apache.org/apacheds/download-old-versions.html[Download Old Versions] page.
