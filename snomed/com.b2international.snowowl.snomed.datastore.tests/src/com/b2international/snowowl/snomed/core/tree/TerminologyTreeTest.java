...
public class TerminologyTreeTest {
	...
	private static class TestTree {
		private final Map<String, SnomedConcept> items = newHashMap();
		...
		public TestTree addNode(String nodeId, boolean primitive, Set<String> parents, Set<String> children) {
			final SnomedConcept entry = new SnomedConcept(nodeId);
			entry.setIconId(Concepts.ROOT_CONCEPT);
			entry.setActive(true);
			entry.setModuleId(Concepts.MODULE_ROOT);
			entry.setDefinitionStatusId(primitive ? Concepts.PRIMITIVE : Concepts.FULLY_DEFINED);
