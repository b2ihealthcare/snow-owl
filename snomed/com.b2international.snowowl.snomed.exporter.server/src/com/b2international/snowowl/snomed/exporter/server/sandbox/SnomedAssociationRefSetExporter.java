...
public class SnomedAssociationRefSetExporter extends SnomedRefSetExporter {
	...
	static {
		...
		fieldsToLoad.add(REFERENCE_SET_MEMBER_TARGET_COMPONENT_ID);
		FIELD_TO_LOAD = unmodifiableSet(fieldsToLoad);
	}
	
	public SnomedAssociationRefSetExporter(final SnomedExportConfiguration configuration, final String refSetId, final SnomedRefSetType type) {
	...
	public String transform(Document doc) {
		...
		sb.append(HT);
		sb.append(doc.get(REFERENCE_SET_MEMBER_TARGET_COMPONENT_ID));
		return sb.toString();
	}
