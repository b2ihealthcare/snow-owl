 ...
 *
 */
public class SnomedSubExporter extends AbstractIterator<String> implements Iterator<String> {
	...
	private Supplier<DocIdsIterator> createDocIdIterator() {
		return memoize(new Supplier<DocIdsIterator>() {
			@Override public DocIdsIterator get() {
				try {
					final DocIdCollector collector = DocIdCollector.create(getIndexSearcher().getIndexReader().maxDoc());
					getIndexSearcher().search(exporter.getExportQuery(branchPath), collector);
	...
	private IndexSearcher getIndexSearcher() {
		...
		if (exporter.getConfiguration().getIndexSearcherMap().containsKey(branchPath.getPath())) {
			return exporter.getConfiguration().getIndexSearcherMap().get(branchPath.getPath());
		}
		
		throw new RuntimeException("Index searcher was not initialized for " + branchPath.getPath());
	}
	
	private Document tryGetDocument(final int docId) {
		try {
			return getIndexSearcher().doc(docId, exporter.getFieldsToLoad());
		} catch (final IOException e) {
