...
public class SnomedCrossMapExporter extends AbstractSnomedCrossMapExporter {
	...
	private static final Set<String> COMPLEX_MEMBER_FIELD_TO_LOAD = Collections.unmodifiableSet(Sets.newHashSet(
			SnomedIndexBrowserConstants.REFERENCE_SET_MEMBER_REFERENCED_COMPONENT_ID,
			SnomedIndexBrowserConstants.REFERENCE_SET_MEMBER_MAP_TARGET_COMPONENT_ID,
			SnomedIndexBrowserConstants.REFERENCE_SET_MEMBER_MAP_PRIORITY,
			SnomedIndexBrowserConstants.REFERENCE_SET_MEMBER_MAP_GROUP,
			...
			try {
				...
				if (null != topDocs && !CompareUtils.isEmpty(topDocs.scoreDocs)) {
					...
					for (int i = 0; i < topDocs.scoreDocs.length; i++) {
						...
						entry.mapSource = doc.get(SnomedIndexBrowserConstants.REFERENCE_SET_MEMBER_REFERENCED_COMPONENT_ID);
