...
//			throw new IllegalArgumentException(message);
	...
	public static Concept buildDraftChildConcept(final SnomedEditingContext editingContext, final Concept parentConcept, @Nullable final String conceptId) {
		...
		for (final Description description : parentConcept.getDescriptions()) {
			...
			final Description newDescription = editingContext.buildDefaultDescription(description.getTerm(), description.getType());
	...
	/**
	 * Returns all the active inbound relationships for a concept.
	 * 
	 * @param destinationConcept
	 *            the container concept
	 *            
	 * @return a list containing all the active inbound relationships
	 */
	public List<Relationship> getActiveInboundRelationships(Concept destinationConcept) {
		return Lists.newArrayList(Iterables.filter(destinationConcept.getInboundRelationships(), new Predicate<Relationship>() {
			@Override public boolean apply(Relationship relationship) {
	...
	public Concept buildDefaultConcept(String fullySpecifiedName, Concept parentConcept) {
		...
		// add FSN
		Description description = buildDefaultDescription(fullySpecifiedName, findConceptById(FULLY_SPECIFIED_NAME));
	...
	public Concept buildDefaultConcept(final String fullySpecifiedName, final String namespace, final Concept moduleConcept, final Concept parentConcept, final Concept... parentConcepts) {
		...
		// set concept properties
		concept.setId(identifiers.generateId(ComponentNature.CONCEPT, namespace));
		concept.setActive(true);
		concept.setDefinitionStatus(findConceptById(PRIMITIVE));
	...
	public Relationship buildEmptyRelationship(final String namespace) {
		...
		relationship.setId(identifiers.generateId(ComponentNature.RELATIONSHIP, namespace));
		return relationship;
	}
	...
	private Description buildDefaultDescription(String term, final String nameSpace, final Concept type, final Concept moduleConcept, final String languageCode) {
		Description description = SnomedFactory.eINSTANCE.createDescription();
		description.setId(identifiers.generateId(ComponentNature.DESCRIPTION, nameSpace));
		description.setActive(true);
