...
public class SnomedURLSchemaSupport implements ResourceURLSchemaSupport {
	...
	public void validate(String uri) throws BadRequestException {
		...
		if (!uri.startsWith(SnomedTerminologyComponentConstants.SNOMED_URI_SCT)) {
			throw new BadRequestException("SNOMED CT URIs must start with '%s'. Got '%s'", SnomedTerminologyComponentConstants.SNOMED_URI_SCT, uri);
		}
	}
